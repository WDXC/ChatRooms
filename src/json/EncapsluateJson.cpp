...
void EncapsluateJson::AppendToObject(const char *lv, const int32_t rv,
                                   Value &val) {
  Value key(lv, m_allocator);
  val.AddMember(key, rv, m_allocator);
}

void EncapsluateJson::AppendToObject(const char *lv, const int64_t rv,
                                   Value &val) {
  Value key(lv, m_allocator);
  val.AddMember(key, rv, m_allocator);
}

void EncapsluateJson::AppendToObject(const char *lv, const uint32_t rv,
                                   Value &val) {
  Value key(lv, m_allocator);
  val.AddMember(key, rv, m_allocator);
}

void EncapsluateJson::AppendToObject(const char *lv, const uint64_t rv,
                                   Value &val) {
  Value key(lv, m_allocator);
  val.AddMember(key, rv, m_allocator);
}

void EncapsluateJson::AppendToObject(const char *lv, const float rv, Value &val) {
  Value key(lv, m_allocator);
  val.AddMember(key, rv, m_allocator);
}

void EncapsluateJson::AppendToObject(const char *lv, const double rv,
                                   Value &val) {
  Value key(lv, m_allocator);
  val.AddMember(key, rv, m_allocator);
}

void EncapsluateJson::AppendToObject(const char *lv, const std::string &rv,
                                   Value &val) {
  Value key(lv, m_allocator);
  Value keyVal;
  keyVal.SetString(rv.c_str(), rv.length(), m_allocator);
...
void EncapsluateJson::AppendToArray(const int32_t lv, Value &rv) {
  rv.PushBack(lv, m_allocator);
}

void EncapsluateJson::AppendToArray(const int64_t lv, Value &rv) {
  rv.PushBack(lv, m_allocator);
}

void EncapsluateJson::AppendToArray(const uint32_t lv, Value &rv) {
  rv.PushBack(lv, m_allocator);
}

void EncapsluateJson::AppendToArray(const uint64_t lv, Value &rv) {
  rv.PushBack(lv, m_allocator);
}

void EncapsluateJson::AppendToArray(const float lv, Value &rv) {
  rv.PushBack(lv, m_allocator);
}

void EncapsluateJson::AppendToArray(const double lv, Value &rv) {
  rv.PushBack(lv, m_allocator);
}

void EncapsluateJson::AppendToArray(const std::string lv, Value &rv) {
  Value keyval;
  keyval.SetString(lv.c_str(), lv.size(), m_allocator);
  rv.PushBack(keyval, m_allocator);
}

void EncapsluateJson::AppendToArray(Value& lv, Value& rv) {
    if (lv.GetType() == kObjectType) {
        rv.PushBack(lv, m_allocator);
    } else {
        lv.PushBack(rv, m_allocator);
    }
}
